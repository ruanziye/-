...
<c:if test="${reader!=null}">
    <form action="${ctxpath}/card" method="post" onsubmit="return confirm('确认执行解除挂失操作？')">
        ...
        <table style="width:600px" class="form-table" align="center">
            ...
            <tr>
                <td class="form-table-btns" colspan="2">
                    <input type="submit" name="save" value="解除挂失" style="margin-right:10px;" ${reader.rdStatus!='挂失'?"disabled":""}>
                    <input type="button" name="cancel" value="取消" onclick="location.href='${ctxpath}/card/restore_card.jsp'">
                </td>
            </tr>
        </table>
        <c:if test="${reader.rdStatus!='挂失'}">
            <center> <font color="red">非挂失状态的借书证，无法进行解除挂失操作</font></center>
        </c:if>
    </form>
</c:if>
</body>
