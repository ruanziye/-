...
<c:if test="${requestScope.result==null||requestScope.result=='1'}">
    ...
    <form action="${ctxpath}/card" method="post" onsubmit="return checkform()">
        <input type="hidden" name="action" value="searchBeforeCancel">
...
<c:if test="${reader!=null}">
    <form action="${ctxpath}/card" method="post" onsubmit="return confirm('确认执行注销操作？')">
        ...
        <table style="width:600px" class="form-table" align="center">
            ...
            <tr>
                <td class="form-table-btns" colspan="2">
                    <input type="submit" name="save" value="注销" style="margin-right:10px;" ${reader.rdStatus!='注销'?"disabled":""}>
                    <input type="button" name="cancel" value="取消" onclick="location.href='${ctxpath}/card/cancel_card.jsp'">
                </td>
            </tr>
        </table>
        <c:if test="${reader.rdStatus!='挂失'}">
            <center> <font color="red">已注销状态的借书证，无法重复进行注销操作</font></center>
        </c:if>
    </form>
</c:if>
</body>
